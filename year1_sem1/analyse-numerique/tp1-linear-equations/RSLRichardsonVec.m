function [x0, it] = RSLRichardsonVec(A, b, x0, alpha, tol, maxIt)
    % Description:
    %   Solve the system of equations A * x = b using the Richardson method. (vectorial variant)
    % Arguments:
    %   A       (M_(n, n))      - matrix.
    %   b       (R^n)           - vector.
    ...
    % Usage:
    %   [x, iterations] = RSLRichardsonVec(A, b[, x0[, alpha[, tol[, maxIt]]])
    % Warning:
