...
void read_nodes(mesh_t *mesh, const char *filename)
{
    ...
    fscanf(fd, "%d %d %d %d\n", &(mesh->nb_nodes), &(mesh->dim), &nb_attrs, &border);
...
void write_data_to_vtk(mesh_t *mesh, const char *dst_file)
{
    ...
    {
        fprintf(fd, "%d ", mesh->cell[0].nb_nodes);
        fprintf(fd, "%d ", mesh->cell[i].node[0]->id);
        fprintf(fd, "%d ", mesh->cell[i].node[1]->id);
        fprintf(fd, "%d\n", mesh->cell[i].node[2]->id);
    }
